import os, re, time, math, json, random, unicodedata, threading, io, hashlib
...
EDGE_TOLERANCE_M = int(os.getenv("EDGE_TOLERANCE_M", "200"))

# Cost caps (adjust via env)
MAX_NEARBY_CALLS = int(os.getenv("MAX_NEARBY_CALLS", "200"))
MAX_NEARBY_PAGES_PER_TILE = int(os.getenv("MAX_NEARBY_PAGES_PER_TILE", "2"))  # 1-3 is sensible
MAX_DETAILS_CALLS = int(os.getenv("MAX_DETAILS_CALLS", "60"))
MAX_SITES_TO_CRAWL = int(os.getenv("MAX_SITES_TO_CRAWL", "40"))  # stop after crawling this many websites

# Early stop if new-unique rate drops below this threshold across a few tiles
EARLY_STOP_MIN_DELTA = int(os.getenv("EARLY_STOP_MIN_DELTA", "2"))
EARLY_STOP_WINDOW = int(os.getenv("EARLY_STOP_WINDOW", "5"))
...
# ==========================
# Simple on-disk caches
# ==========================
CACHE_DIR = os.getenv("CACHE_DIR", ".cache")
os.makedirs(CACHE_DIR, exist_ok=True)
NEARBY_CACHE_PATH = os.path.join(CACHE_DIR, "nearby_cache.json")
DETAILS_CACHE_PATH = os.path.join(CACHE_DIR, "details_cache.json")
...
NEARBY_CACHE = _load_cache(NEARBY_CACHE_PATH)
DETAILS_CACHE = _load_cache(DETAILS_CACHE_PATH)
...
def cache_key_nearby(center, radius_m, types):
    lat, lon = center
    key = f"{round(lat,6)},{round(lon,6)}:{radius_m}:{','.join(sorted(types))}"
    return hashlib.sha1(key.encode()).hexdigest()

def cache_get_nearby(center, radius_m, types):
    return NEARBY_CACHE.get(cache_key_nearby(center, radius_m, types))

def cache_set_nearby(center, radius_m, types, results):
    NEARBY_CACHE[cache_key_nearby(center, radius_m, types)] = results
    _save_cache(NEARBY_CACHE_PATH, NEARBY_CACHE)

def cache_get_details(place_id):
    return DETAILS_CACHE.get(place_id)

def cache_set_details(place_id, data):
    DETAILS_CACHE[place_id] = data
...
def geocode_viewport(gmaps_client, place_text: str):
    ...
    dlat, dlon = km_to_deg(lat, 1.0)  # tiny fallback box (~1 km half-size)
...
# ==========================
# Google Places (cost-aware)
# ==========================
nearby_calls = 0
details_calls = 0

def fetch_nearby_all_pages(gmaps_client, center, radius_m, types=("dentist","dental_clinic")):
    global nearby_calls
    # Check cache first
    ck = cache_get_nearby(center, radius_m, types)
    if ck is not None:
        return ck
    ...
    cache_set_nearby(center, radius_m, types, out)
    return out

def gmaps_place_details(gmaps_client, place_id):
    global details_calls
    cached = cache_get_details(place_id)
    if cached:
        return {"result": cached}

    if details_calls >= MAX_DETAILS_CALLS:
        # Return minimal stub; we won't crawl without website
    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            cache_set_details(place_id, {
                ...
                "place_id": r.get("place_id") or place_id
            })
            return {"result": cache_get_details(place_id)}
        except Exception:
            time.sleep(0.6 * (2 ** attempt) + random.random() * JITTER)
    return {"result": {"place_id": place_id}}
    ...
    children=[
        ...
        html.Div([
            ...
            html.Div([html.Label("Max tiles center-out"),
                      dcc.Input(id="max_tiles", type="number", value=30, step=1)]),
            html.Div([html.Label("Max clinics to collect"),
                      dcc.Input(id="max_total_places", type="number", value=300, step=50)]),
        ], style={"marginTop":"8px","display":"grid","gridTemplateColumns":"repeat(4, minmax(180px,1fr))","gap":"10px"}),
        html.Div([
            html.Div([html.Label("Max pages per site (crawl)"),
                      dcc.Input(id="max_pages_per_site", type="number", value=60, step=5)]),
            html.Div([html.Label("Max seconds per site (crawl)"),
...
def run_job(args):
    ...
    try:
        ...
        radius_km = float(args.get("radius_km", 0.5))
        step_factor = float(args.get("step_factor", 1.5))
        max_tiles = int(args.get("max_tiles", 30))
        max_total_places = int(args.get("max_total_places", 300))
        max_pages_per_site = int(args.get("max_pages_per_site", 60))
        ...
        set_job(diag=f"Tiles planned {len(centers)} • Viewport ~{cover_w_km:.1f}×{cover_h_km:.1f} km")
        ...
        # Per-run domain dedupe: don’t crawl same corporate site 20 times
        seen_domains = set()
        sites_crawled = 0

        def worker(pid):
            nonlocal sites_crawled
...
def start(n, place_text, radius_km, step_factor, max_tiles, max_total_places,
    ...
    args = {
        ...
        "radius_km": float(radius_km) if radius_km is not None else 0.5,
        "step_factor": float(step_factor) if step_factor is not None else 1.5,
        "max_tiles": int(max_tiles) if max_tiles is not None else 30,
        "max_total_places": int(max_total_places) if max_total_places is not None else 300,
        "max_pages_per_site": int(max_pages_per_site) if max_pages_per_site is not None else 60,
